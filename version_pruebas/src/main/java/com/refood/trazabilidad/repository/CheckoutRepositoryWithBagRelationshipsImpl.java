...
public class CheckoutRepositoryWithBagRelationshipsImpl implements CheckoutRepositoryWithBagRelationships {
    ...
    public Optional<Checkout> fetchBagRelationships(Optional<Checkout> checkout) {
        return checkout.map(this::fetchAlSals);
    }
    ...
    public List<Checkout> fetchBagRelationships(List<Checkout> checkouts) {
        return Optional.of(checkouts).map(this::fetchAlSals).orElse(Collections.emptyList());
    }

    Checkout fetchAlSals(Checkout result) {
        return entityManager
            .createQuery(
                "select checkout from Checkout checkout left join fetch checkout.alSals where checkout is :checkout",
                Checkout.class
            )
    ...
    List<Checkout> fetchAlSals(List<Checkout> checkouts) {
        ...
        List<Checkout> result = entityManager
            .createQuery(
                "select distinct checkout from Checkout checkout left join fetch checkout.alSals where checkout in :checkouts",
                Checkout.class
            )
