...
public class TipoAlResource {
    ...
    @GetMapping("/tipo-al-all")
    public ResponseEntity<List<TipoAl>> getAll() {
        List<TipoAl> tipoDeAlimentos = tipoAlService.findAll();
     ...
     *         body the new tipoAlDTO, or with status {@code 400 (Bad Request)} if
     *         the tipoAl has already an ID.
     ...
     *         the updated tipoAlDTO,
     *         or with status {@code 400 (Bad Request)} if the tipoAlDTO is not
     *         valid,
     *         or with status {@code 500 (Internal Server Error)} if the tipoAlDTO
     *         couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    ...
    public ResponseEntity<TipoAlDTO> updateTipoAl(
            ...
            @Valid @RequestBody TipoAlDTO tipoAlDTO) throws URISyntaxException {
        ...
        return ResponseEntity
                ...
                .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME,
                        tipoAlDTO.getId().toString()))
                .body(result);
    }

    /**
     * {@code PATCH  /tipo-als/:id} : Partial updates given fields of an existing
     * tipoAl, field will ignore if it is null
     *
     * @param id        the id of the tipoAlDTO to save.
     * @param tipoAlDTO the tipoAlDTO to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
    ...
    public ResponseEntity<TipoAlDTO> partialUpdateTipoAl(
            @PathVariable(value = "id", required = false) final Long id,
            @NotNull @RequestBody TipoAlDTO tipoAlDTO) throws URISyntaxException {
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the tipoAlDTO, or with status {@code 404 (Not Found)}.
     */
