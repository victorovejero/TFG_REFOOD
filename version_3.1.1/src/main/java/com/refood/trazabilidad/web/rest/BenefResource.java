...
public class BenefResource {
    ...
    @GetMapping("/benef-all")
    public ResponseEntity<List<Benef>> getAll() {
        List<Benef> beneficiarios = benefService.findAll();
     ...
     *         body the new benefDTO, or with status {@code 400 (Bad Request)} if
     *         the benef has already an ID.
     ...
     *         the updated benefDTO,
     *         or with status {@code 400 (Bad Request)} if the benefDTO is not
     *         valid,
     *         or with status {@code 500 (Internal Server Error)} if the benefDTO
     *         couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    ...
    public ResponseEntity<BenefDTO> updateBenef(
            ...
            @Valid @RequestBody BenefDTO benefDTO) throws URISyntaxException {
        ...
        return ResponseEntity
                ...
                .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME,
                        benefDTO.getId().toString()))
                .body(result);
    }

    /**
     * {@code PATCH  /benefs/:id} : Partial updates given fields of an existing
     * benef, field will ignore if it is null
     *
     * @param id       the id of the benefDTO to save.
     * @param benefDTO the benefDTO to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
    ...
    public ResponseEntity<BenefDTO> partialUpdateBenef(
            @PathVariable(value = "id", required = false) final Long id,
            @NotNull @RequestBody BenefDTO benefDTO) throws URISyntaxException {
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the benefDTO, or with status {@code 404 (Not Found)}.
     */
