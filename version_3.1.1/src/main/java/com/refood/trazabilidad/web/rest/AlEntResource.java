...
public class AlEntResource {
    ...
    @GetMapping("/al-ent-all")
    public ResponseEntity<List<AlEnt>> getAll() {
        List<AlEnt> alimentoDeEntrada = alEntService.findAll();
     ...
     *         body the new alEntDTO, or with status {@code 400 (Bad Request)} if
     *         the alEnt has already an ID.
     ...
     *         the updated alEntDTO,
     *         or with status {@code 400 (Bad Request)} if the alEntDTO is not
     *         valid,
     *         or with status {@code 500 (Internal Server Error)} if the alEntDTO
     *         couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    ...
    public ResponseEntity<AlEntDTO> updateAlEnt(
            ...
            @Valid @RequestBody AlEntDTO alEntDTO) throws URISyntaxException {
        ...
        return ResponseEntity
                ...
                .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME,
                        alEntDTO.getId().toString()))
                .body(result);
    }

    /**
     * {@code PATCH  /al-ents/:id} : Partial updates given fields of an existing
     * alEnt, field will ignore if it is null
     *
     * @param id       the id of the alEntDTO to save.
     * @param alEntDTO the alEntDTO to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
    ...
    public ResponseEntity<AlEntDTO> partialUpdateAlEnt(
            @PathVariable(value = "id", required = false) final Long id,
            @NotNull @RequestBody AlEntDTO alEntDTO) throws URISyntaxException {
     ...
     */
    @GetMapping("/al-ents")
    public ResponseEntity<List<AlEntDTO>> getAllAlEnts(
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the alEntDTO, or with status {@code 404 (Not Found)}.
     */
