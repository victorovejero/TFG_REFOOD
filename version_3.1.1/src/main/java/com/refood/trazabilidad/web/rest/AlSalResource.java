...
public class AlSalResource {
     ...
     * @return the {@link ResponseEntity} with status {@code 201 (Created)} and with body the new alSalDTO, or with status {@code 400 (Bad Request)} if the alSal has already an ID.
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated alSalDTO,
     * or with status {@code 400 (Bad Request)} if the alSalDTO is not valid,
     * or with status {@code 500 (Internal Server Error)} if the alSalDTO couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping("/al-sals/{id}")
    public ResponseEntity<AlSalDTO> updateAlSal(
        @PathVariable(value = "id", required = false) final Long id,
        @Valid @RequestBody AlSalDTO alSalDTO
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, alSalDTO.getId().toString()))
            .body(result);
    }

    /**
     * {@code PATCH  /al-sals/:id} : Partial updates given fields of an existing alSal, field will ignore if it is null
     *
     * @param id the id of the alSalDTO to save.
     * @param alSalDTO the alSalDTO to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated alSalDTO,
    ...
    public ResponseEntity<AlSalDTO> partialUpdateAlSal(
        @PathVariable(value = "id", required = false) final Long id,
        @NotNull @RequestBody AlSalDTO alSalDTO
    ) throws URISyntaxException {
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of alSals in body.
     */
    @GetMapping("/al-sals")
    public ResponseEntity<List<AlSalDTO>> getAllAlSals(@org.springdoc.api.annotations.ParameterObject Pageable pageable) {
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the alSalDTO, or with status {@code 404 (Not Found)}.
     */
